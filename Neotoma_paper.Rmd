...
```{r fig-3-alnus-data-plot, dpi=150, fig.ext='png'}
...
It is also possible to plot the pollen stratigraphy (Figure 4) at any one site, again, using the `analogue` package for R [@analogue2007].  Here we plot Marion Lake:

```{r, fig-4-marion-plot, fig.height=3, fig.width=6, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r fig-5-map-pinus-example, echo = TRUE, message = FALSE, warning = FALSE}
map <- map_data('world')
...
```{r fig-6-pinus-recal-plot, echo = TRUE, fig.width = 6, fig.height = 3, warning = FALSE}
