...
# i like to see the provincial borders...
bc.map <- get_map(location = c(-120, 60), zoom = 4)
ggmap(bc.map) + geom_point(data = all.sites, aes(x = long, y = lat)) + 
  geom_point(data = get_site(dataset = all.datasets), aes(x = long, y = lat), color = 2) + 
...
top.pinus <- function(x){
  ...
  oldest.row <- if (any(x.pct[, 'Pinus'] > .05) ) { max(which(x.pct[, 'Pinus'] > .05)) } else {oldest.row=0}
...
# why the heck is the next line in here?
...
# To make side-by-side plots, can use multiplot
multiplot(mapped, regress, cols=2) # we can get rid of grid.arrange if we do this...
