...
compiled.cores <- lapply(all.downloads, function(x) compile_taxa(x, 'P25'))
...
The function `compile_taxa` returns an object that looks exactly like the `download` passed to it, however, the `taxon.list` data.frame gains a column named `compressed` that links the original taxonomy to the revised taxonomy.  This acts as an important check for researchers who choose to use this package for large-scale analysis.  Here we see that taxa such as *Potentilla* is lumped into `Other`, along with spores and other taxa.  The `compile_taxa` function can also accept user-defined tables for aggregation if the provided compilations are not acceptable.
