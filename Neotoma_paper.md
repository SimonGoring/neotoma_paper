...
The Neotoma Paleoecological Database was named for the packrat genus *Neotoma*, because this genus collects material from the environment around its midden
...
```
#> The API call was successful, you have returned  433 records.
```
...
Here the API tells us we now have only 59 records of the original 433.  Many of the samples are pollen surface samples, or vertebrate fauna, meaning pollen core data comprises less than half of the records.  Regardless, we now know that there is pollen core data from 59 sites and we can plot those sites over our original 433.
...
ggplot(data = data.frame(map), aes(long, lat)) + geom_polygon(aes(group = group), 
    ...
    y = lat)) + geom_point(data = get_site(dataset = all.datasets), aes(x = long, 
    y = lat), color = 2) + xlab("Longitude West") + ylab("Latitude North") + 
    coord_map(projection = "albers", lat0 = 40, lat1 = 65, xlim = c(-140, -110), 
        ylim = c(40, 70))
```
...
#> |:--|:------------------|:-------------|:---------------|:---------------|:-------------------|
#> |2  |Lycopodium tablets |grains/tablet |concentration   |NA              |Laboratory analyses |
#> |24 |Picea undiff.      |NISP          |pollen          |NA              |Vascular plants     |
#> |3  |Betula             |NISP          |pollen          |NA              |Vascular plants     |
#> |4  |Lycopodium spike   |number        |counted         |NA              |Laboratory analyses |
#> |5  |Poaceae            |NISP          |pollen          |NA              |Vascular plants     |
#> |6  |Ericaceae          |NISP          |pollen          |NA              |Vascular plants     |
```
...
#> |:--|:------------------|:-------------------|:----------|
#> |2  |Lycopodium tablets |Laboratory analyses |NA         |
#> |24 |Picea undiff.      |Vascular plants     |Picea      |
#> |3  |Betula             |Vascular plants     |Betula     |
#> |4  |Lycopodium spike   |Laboratory analyses |NA         |
#> |5  |Poaceae            |Vascular plants     |Poaceae    |
#> |6  |Ericaceae          |Vascular plants     |Other      |
```
...
top.pinus <- function(x) {
    ...
    data.frame(site = x$metadata$site.data$sitename, lat = x$metadata$site.data$lat, 
...
mapped <- ggplot(data = data.frame(map), aes(long, lat)) + geom_polygon(aes(group = group), 
    ...
    y = lat, color = age), size = 3) + coord_map(projection = "albers", lat0 = 40, 
