...
To begin we must define a spatial bounding box and a set of taxa of interest.  Strong and Hills (2013) use a region approximately bounded by 54^{o}N to the south and 65^{o}N to the North, and from 110^{o}W to 130^{o}W.  The command \texttt{get\_site} is used to find all sites within a bounding box:
...
The \texttt{get\_sites} command returns a site \texttt{data.frame}, with siteID, latitude, longitude, altitude, SiteName, and SiteDescription.  Each row represents a unique site. 

We can see that this returns a total of \Sexpr{nrow(all.sites)} sites.  Sites are effectively containers for datasets though.  Generally it's better to search for datasets.  When you search for a dataset you can limit the type of dataset, either by looking for specific taxa, or by describing the dataset type.  Here we will look for all taxa beginning with \emph{Pinus} in pollen dataset.  We use the \texttt{*} wildcard to indicate any and all taxa with \emph{Pinus} in their name:
...
Here the API tells us we now have only \Sexpr{length(all.datasets)} records of the original \Sexpr{nrow(all.sites)}.  Many of the samples are pollen surface samples, or vertebrate fauna, meaning pollen core data comprises less than half of the records.  Regardless, we now know that there is pollen core data from \Sexpr{length(all.datasets)} sites and we can plot those sites over our original \Sexpr{nrow(all.sites)}.
...
In most cases the \texttt{get\_download} command will return a message for an individual core such as:

<<eval=FALSE>>=
API call was successful. Returned record for Cottonwood Slough
@

The \texttt{download} object is a list with six objects.  The \texttt{metadata} is again a list with a \texttt{dataset}, similar to the one returned by \texttt{get_dataset} and then \texttt{pi.data}, information about the investigator.  The \texttt{sample.meta} is where the depth and age information is stored. The actual chronologies are stored in the \texttt{chronology} list.  If a core has a single record the list has a length of one.  Some cores have multiple chronologies and these are added to the list.  The default chronology is always represented in \texttt{sample.meta}, and is always the first \texttt{chronology}.  If you choose to build your own chronology using Bacon (Blaauw and Christen, 2011) or another method you can obtain the chronological controls for the core using the \texttt{get\_chroncontrol} function and the chronology ID in either \texttt{sample.meta} or any one of the \texttt{chronology} objects, however the chronological controls used to build a chronology may vary.  Often the default model contains the most accurate chronological control data.

The \texttt{taxon.list} is a critical component of a \texttt{download} object.  It lists the taxa found in the core, as well as any laboratory data, along with the units of measurement and taxonomic grouping.  This is important information for determining which taxa make it into pollen percentages. The \texttt{counts} are the actual count or percentage data recorded for the core.  The \texttt{lab.data} contains information about any spike used to determine concentrations, sample quantities and, in some cases, charcoal counts.

We have \Sexpr{length(all.downloads)} records in our analysis. The pollen taxonomy can vary substantially across cores often depending on researcher skill, or changing taxonomies for species, genera or families over time.  This shifting taxonomy is often problematic to deal with.  The \texttt{neotoma} package implements a taxonomic standardizer to attempt to standardize to one of four published taxonomies for the United States and Canada.  While this function can be helpful in many cases it should also be used with care.  The aggregation table is accessible using \texttt{data(pollen.equiv)} and the function to compile the data is called \texttt{compile_list}.
...
This core, from \Sexpr{compiled.cores[[1]]$metadata$site.data$SiteName} shows changes through time, particularly for \emph{Betula} and \emph{Alnus}, but little \emph{Pinus} pollen.
...
And so we see a clear pattern of migration by \emph{Pinus} in northwestern North America.  These results match up broadly with the findings of Strong and Hills (2013) who suggest that \emph{Pinus} reached a northern extent between 59^o and 60^oN at approximately 7 - 10kyr as a result of geographic barriers.

\subsection*{\emph{Phenacomy intermedius} Migration}
Phenacomy intermedius is a species of vole found in western North America from New Mexico to the Yukon Territory of Canada.  P. intermedius lives underground in burrows or sub-nivian tunnels.

P. intermedius is found
<<>>=
pinter.sites <- get_dataset(taxonname='Phenacomy*')
pinter.down <- get_download(sapply(pinter.sites, function(x)x$DatasetID))
### Note issue here with Mammuthus for Owl Cave and others.
